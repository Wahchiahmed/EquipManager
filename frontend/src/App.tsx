...
// Renders a different component depending on whether the user is ADMIN or not.
// Prevents role-gated API calls (e.g. /stock/*) from firing for the wrong role.
...
const AppRoutes = () => {
  ...
  return (
    <Routes>
      ...
      {/* ── Employee routes ────────────────────────────────────────────────
           Each page is its own component that only calls /demandes & /produits,
           which are open to all authenticated roles — no 403 risk.           */}
      <Route
        path="/my-requests"
        element={
          <ProtectedLayout>
      ...
      {/* ── Shared: /all-requests
            Manager → ManagerDashboard       (/dept/demandes)
            Admin   → AdminToutesLesDemandes (/admin/demandes)               */}
      <Route
        path="/all-requests"
        element={
          <ProtectedLayout>
      ...
      {/* ── Shared: /products
            Stock → StockDashboard    (/stock/produits — role-gated)
            Admin → AdminProduitsStock (/produits — open)                     */}
      <Route
        path="/products"
        element={
          <ProtectedLayout>
      ...
      {/* ── Admin-only routes (dedicated URLs, no overlap with other roles) ── */}
      <Route
        path="/users"
        element={
          <ProtectedLayout>
