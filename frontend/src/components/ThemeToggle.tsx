 ...
 *
 * Two variants:
 *   <ThemeToggle />              → dropdown (default) — for Navbar
 *   <ThemeToggle variant="icon" /> → cycle icon — compact use
 */
...
const OPTIONS: { key: ThemeMode; label: string; icon: React.FC<{ className?: string }> }[] = [
...
// ── Variant: icon only (cycles on click) ──────────────────────────────────────
...
const DropdownToggle: React.FC = () => {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div
          className="absolute right-0 mt-2 w-44 bg-card border border-border
